...
graph TB
    subgraph Project["🏦 finance-tracker-india"]
        direction TB
        ...
        subgraph Frontend["📱 Frontend - Next.js + TypeScript"]
            ...
            subgraph Pages["Pages (src/app)"]
                P1["page.tsx<br/>Landing Page"]
                P2["login/page.tsx<br/>Login"]
                P3["signup/page.tsx<br/>Signup"]
                P4["dashboard/page.tsx<br/>Dashboard"]
                P5["expenses/page.tsx<br/>Expense List"]
                P6["expenses/add/page.tsx<br/>Add Expense"]
                P7["insights/page.tsx<br/>AI Insights"]
            end

            subgraph Components["Components (src/components)"]
                C1["Navbar.tsx"]
                C2["Sidebar.tsx"]
                subgraph UI["UI Primitives (ui/)"]
                    U1["badge.tsx"]
                    U2["button.tsx"]
                    U3["card.tsx"]
                    U4["input.tsx"]
            ...
            subgraph Lib["Lib (src/lib)"]
                L1["api.ts<br/>API Client"]
                L2["auth.ts<br/>Auth Helper"]
                L3["supabase.ts<br/>Supabase Client"]
                L4["utils.ts<br/>Utilities"]
            end

            MW["middleware.ts<br/>Route Protection"]
            GC["globals.css<br/>Global Styles"]
            LY["layout.tsx<br/>Root Layout"]

            subgraph FConfig["Config Files"]
                FC1["next.config.js"]
                FC2["tailwind.config.js"]
                FC3["postcss.config.js"]
                FC4["tsconfig.json"]
                FC5["package.json"]
            end
        end
        ...
        subgraph Backend["⚙️ Backend - FastAPI + Python"]
            ...
            subgraph Routes["Routes (app/routes)"]
                R1["expenses.py<br/>CRUD Endpoints"]
                R2["analysis.py<br/>Analysis API"]
                R3["unified_analysis.py<br/>Unified Insights"]
                R4["user.py<br/>Auth Endpoints"]
            end

            subgraph Models["Models (app/models)"]
                M1["expense.py<br/>Expense Model"]
                M2["user.py<br/>User Model"]
            end

            subgraph DB["Database (app/db)"]
                D1["database.py<br/>DB Config"]
                D2["session.py<br/>Session Mgmt"]
                D3["supabase_client.py<br/>Supabase Auth"]
            end

            subgraph Services["Services (app/services)"]
                S1["financial_engine.py<br/>Core Finance Logic"]
                S2["rule_engine.py<br/>Savings Rules"]
                S3["optimizer.py<br/>Spend Optimizer"]
                S4["inflation_engine.py<br/>Inflation Analysis"]
                S5["cpi_service.py<br/>CPI Data"]
                S6["rbi_service.py<br/>RBI Rates"]
                S7["fuel_service.py<br/>Fuel Prices"]
                S8["deals_service.py<br/>Deal Finder"]
            end

            subgraph Utils["Utils (app/utils)"]
                UT1["helpers.py"]
                UT2["scoring.py<br/>Financial Score"]
            end

            SCH["scheduler.py<br/>Background Jobs"]
            REQ["requirements.txt"]
            DBFILE["finance_tracker.db<br/>SQLite DB"]
        end
    end
    ...
    P2 & P3 -->|Auth| L2
    L2 -->|Supabase Auth| L3
    P4 & P5 & P6 & P7 -->|API Calls| L1
    L1 -->|HTTP| MAIN
    MW -->|Guards| P4 & P5 & P6 & P7
    ...
    SCH -->|Scheduled Tasks| S5 & S6 & S7

    LY --> C1 & C2
    P4 & P5 & P7 --> U1 & U2 & U3 & U4

    %% ---- Styling ----
    classDef frontend fill:#1e3a5f,stroke:#4a90d9,color:#e0e0e0
    classDef backend fill:#3b1f2b,stroke:#d94a7a,color:#e0e0e0
    classDef service fill:#2d4a1e,stroke:#7ad94a,color:#e0e0e0
    classDef db fill:#4a3b1e,stroke:#d9b44a,color:#e0e0e0
    classDef config fill:#333,stroke:#888,color:#aaa

    class P1,P2,P3,P4,P5,P6,P7,C1,C2,U1,U2,U3,U4,MW,GC,LY frontend
    class L1,L2,L3,L4 frontend
    class MAIN,R1,R2,R3,R4,M1,M2,SCH,REQ backend
    class S1,S2,S3,S4,S5,S6,S7,S8,UT1,UT2 service
    class D1,D2,D3,DBFILE db
    class FC1,FC2,FC3,FC4,FC5 config
